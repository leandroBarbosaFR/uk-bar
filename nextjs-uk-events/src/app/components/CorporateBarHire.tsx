...
import '../styles/corporateBarHire.css'
import PortableTextRenderer from './PortableTextRenderer'

const CORPORATE_BAR_HIRE_QUERY = `*[_type == "corporateBarHire"][0]{
  title,
  subtitle,
  body,
  image{
...
export default function CocktailBarHire() {
  const [data, setData] = useState<CorporateBarHireData | null>(null)

  useEffect(() => {
    async function fetchData() {
      const result = await client.fetch(CORPORATE_BAR_HIRE_QUERY)
      setData(result)
    }
    fetchData()
  }, [])
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            ...
            {data.image?.asset?.url && (
              <div className="image-wrapper-corporate">
                <Image
                  width={1200}
                  height={800}
                  src={urlFor(data.image).width(1200).url()}
                  alt={data.image?.alt || 'Corporate bar image'}
